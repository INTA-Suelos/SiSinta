#!/usr/bin/env Rscript
# Uso: slabs.R perfiles.csv nombre.png
...
sisinta <- sisinta[!is.na(sisinta$profundidad_superior) &
                     !is.na(sisinta$analitico_arcilla) &
                     !is.na(sisinta$analitico_carbono_organico_c) &
                     !is.na(sisinta$analitico_ph_h2o),]
...
suelos$CLAY <- suelos$arcilla
suelos$SILT <- suelos$limo_50
suelos$SAND <- 100 - suelos$CLAY - suelos$SILT
suelos$SOC <- suelos$CO
suelos$pH <- suelos$pH_H2O

if(length(suelos)>0){agg <- aqp::slab(suelos, fm= ~ SOC + pH + CLAY + SILT + SAND)
}
png(filename=as.character(args[2]), width = 900, height = 480)
if(length(suelos)>0){xyplot(top ~ p.q50 | variable, data=agg, ylab='Depth',
