...
describe Deserializador::Constructor do
  ...
  let(:atributos_de_analitico) do
    %w{
      ...
      base_ca base_mg base_k base_na arcilla carbono_organico_c
      carbono_organico_n limo_2_20 limo_2_50 arena_muy_fina arena_fina
      arena_media arena_gruesa arena_muy_gruesa ca_co3 agua_15_atm
      agua_util conductividad h saturacion_t saturacion_s_h
      densidad_aparente profundidad_muestra agua_3_atm carbono_organico_cn
    }
  end
  ...
  describe '#actualizar?' do
    it 'crea los datos por omisión' do
      _(constructor_creador.actualizar?).must_equal false
    end

    it 'registra si hay que actualizar' do
      _(constructor_actualizador.actualizar?).must_equal true
    end
  end

  describe '#construir_perfil' do
    it 'es nuevo por omisión' do
      _(constructor_creador.construir_perfil).wont_be :persisted?
    end
    ...
    it 'si hay que actualizar carga el existente' do
      ...
      _(perfil).must_be :persisted?
      _(perfil).must_equal perfil_existente
    end
    ...
    it 'no actualiza el usuario' do
      ...
      _(perfil.usuario_id).wont_equal perfil_cambiado.usuario_id
      _(perfil.usuario_id).must_equal perfil_existente.usuario_id
    end
  end
